...
// Stack Layout implementation for ImGui (PR #846 by thedmd).
// Source: https://github.com/thedmd/imgui/tree/feature/layout
//
// Provides ImGui::BeginHorizontal / EndHorizontal, BeginVertical / EndVertical,
// Spring, SuspendLayout, ResumeLayout.
//
// NOTE: imgui_internal.h must have been patched by CMakeLists.txt before this
// translation unit is compiled (adds ImGuiLayout, ImGuiLayoutItem structs and
// the CurrentLayout / LayoutStack / Layouts fields to ImGuiWindowTempData).
...
// ---------------------------------------------------------------------------
// Forward declarations (all helpers are file-scoped within the ImGui namespace)
// ---------------------------------------------------------------------------
namespace ImGui
{
...
{
    // Accumulate amount of occupied space and spring weights
...
static void ImGui::BeginLayoutItem(ImGuiLayout& layout)
{
    ...
    // If layout changes in current frame alignment will be corrected in EndLayout().
...
static void ImGui::AddLayoutSpring(ImGuiLayout& layout, float weight, float spacing)
{
    ...
    previous_item = NULL; // may be invalidated after GenerateLayoutItem()
    ...
    if (spacing < 0.0f)
    {
        ...
        spacing = (layout.Type == ImGuiLayoutType_Horizontal) ? style_spacing.x : style_spacing.y;
    }
...
// Inserts a spring (elastic spacer) into the current layout.
//   weight <= 0  : always zero size
//   weight > 0   : proportional share of free space
//   spacing < 0  : use style.ItemSpacing on the layout axis
//   spacing >= 0 : enforce this spacing amount
